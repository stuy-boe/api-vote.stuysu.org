...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    throw new RequestRefusalError(
        'Start date/time or end date/time is not valid.', 'INVALID_TIMES');
  }
  ...
  if (!name) {
    throw new RequestRefusalError(
        'A valid name is required to create an election.', 'INCOMPLETE_FORM');
  }

  if (!electionTypes.includes(type)) {
    throw new RequestRefusalError('That election type is not supported',
  ...
  if (!publicUrl || publicUrlExists) {
    throw new RequestRefusalError(
        'There already exists an election with that public url.', 'URL_EXISTS');
  }
  ...
  for (let x = 0; x < grades.length; x++) {
    ...
    if (allGrades.includes(grade)) {
      await allowedGrades.create(
          {electionId : election.id, grade : String(grade)});
    }
  }

  res.json({
    success : true,
    payload : {election : {id : election.id, publicUrl : election.publicUrl}}
  });
});
