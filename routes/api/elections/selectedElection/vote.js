...
router.use('*', async (req, res, next) => {
  if (!req.session.signedIn) {
    throw new RequestRefusalError('You need to be signed in to vote.',
                                  'AUTH_REQUIRED');
  }
  ...
  if (!(await req.election.includesGrade(grade))) {
    throw new RequestRefusalError(
        'You are not allowed to vote for this election.', 'NOT_ALLOWED');
  }

  if (await req.election.existsVote(req.session.getDecryptedUserId())) {
    throw new RequestRefusalError('You have already voted for this election.',
                                  'ALREADY_VOTED');
  }
...
router.post('/', async (req, res) => {
  ...
  for (let choiceNumber = 0; choiceNumber < choices.length; choiceNumber++) {
    ...
    await voteData.create({voteId, choiceNumber, data : choice});
  }
