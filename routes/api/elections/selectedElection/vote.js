...
router.use("*", async (req, res, next) => {
	...
	if( ! req.election.isVotingPeriod() ){
		throw new RefusalError("It is not currently the voting period for this election.", "NOT_VOTING_PERIOD");
	}

	const grade = students.getGrade(req.session.email);

	if( ! grade ){
		throw new RefusalError("Your grade is unknown and so you are not yet allowed to vote.", "GRADE_UNKNOWN");
	}

	if( ! await req.election.includesGrade(grade) ){
		throw new RefusalError("You are not allowed to vote for this election.", "NOT_ALLOWED");
	}

	if( await req.election.existsVote(req.session.getDecryptedUserId()) ){
		throw new RefusalError("You have already voted for this election.", "ALREADY_VOTED");
	}
...
router.post("/", async (req, res) => {
	...
	if( ! Array.isArray(choices) ){
		throw new RefusalError("The choices were not sent in a way that the server can understand.", "INVALID_FORMAT");
	}

	if( ! Boolean(choices.length) ) {
		throw new RefusalError("You must choose at least one candidate to vote for.", "INSUFFICIENT_SELECTIONS");
	}
