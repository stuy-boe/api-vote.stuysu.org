...
describe('sessionValidator', () => {
  ...
  before(() => {
    ...
    app.get('/mock-session', (req, res) => {
      ...
      req.session.encryptedUserId = encryptString(originalUserId, key, iv);
  ...
  beforeEach(async () => {
    ...
    cookies = Object.keys(cookieData)
                  .map(name => `${name}=${cookieData[name].value}`);
  });
  ...
  describe('session checking', () => {
    before(() => {
      app.get(
          '/check-session',
          (req,
           res) => { res.json({signedIn : Boolean(req.session.signedIn)}); });
    });

    it('should not affect session if all decryption and session cookies are valid',
       done => {
         request(app).get('/check-session').set('Cookie', cookies).then(res => {
           expect(res.body.signedIn).to.be.true;
           done();
         });
       });

    it('should force session expiration if decryptKey cookie is changed',
       done => {
    ...
    it('should log me out if decryptIv cookie is changed', done => {
      ...
      request(app).get('/check-session').set('Cookie', newCookies).then(res => {
        expect(res.body.signedIn).to.be.false;
        done();
      });
    });
  ...
  it('should add getDecryptedUserId method to session if signed in', done => {
    app.get(
        '/decryptUserId',
        (req,
         res) => { res.json({userId : req.session.getDecryptedUserId()}); });

    request(app).get('/decryptUserId').set('Cookie', cookies).then(res => {
  ...
  describe('Voting Station', () => {
    it('should extend my session expiration on future requests if not a voting station',
       done => {
         setTimeout(() => {
           request(app)
               .get('/check-session')
               ...
               .then(res => {
                 ...
                 expect(newCookieData.decryptKey.expires.getTime())
                     .to.be.gt(cookieData.decryptKey.expires.getTime());

                 expect(newCookieData.decryptIv.expires.getTime())
    ...
    it('should not extend my session if a voting station cookie is present',
       done => {
         const newCookies = [...cookies, 'isVotingStation=true' ];

         request(app).get('/api/state').set('Cookie', newCookies).then(res => {
           const renewedCookieData = setCookie(
               res.header['set-cookie'], {decodeValues : true, map : true});

           expect(renewedCookieData).to.not.have.property('decryptKey');
  ...
  describe('NODE_ENV is production', () => {
    ...
    it('should deliver cookies with secure flag enabled and sameSite set to none',
       done => {
         request(app).get('/api/state').set('Cookie', cookies).then(res => {
           const renewedCookieData = setCookie(
               res.header['set-cookie'], {decodeValues : true, map : true});
           ...
           expect(renewedCookieData.decryptKey.sameSite).to.equal('None');
           expect(renewedCookieData.decryptIv.sameSite).to.equal('None');
  ...
  describe('NODE_ENV is not production', () => {
    ...
    it('should not explicitly declare ssl for cookies', done => {
      request(app).get('/api/state').set('Cookie', cookies).then(res => {
        const renewedCookieData = setCookie(res.header['set-cookie'],
                                            {decodeValues : true, map : true});

        expect(renewedCookieData.decryptKey).to.not.have.property('secure');
        expect(renewedCookieData.decryptIv).to.not.have.property('secure');
