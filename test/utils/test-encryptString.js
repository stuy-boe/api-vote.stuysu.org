...
describe('encryptString', () => {
  it('should return the same hex when called multiple times with the same parameters',
     done => {
  ...
  it('should reject keys with the wrong number of bytes', done => {
    ...
    expect(encryptStringBound)
        .to.throw(Error, 'The key must be a 32 byte Buffer.');

    done();
  });

  it('should reject initialization vectors of the wrong number of bytes',
     done => {
       const key = crypto.randomBytes(32);
